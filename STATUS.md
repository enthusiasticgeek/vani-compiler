...
**Last updated:** 2026-06-01 (closures #520-#523 — **`vec_sliding_max`**, **`vec_sliding_min`**, **`vec_sliding_sum`**, **`vec_sliding_product`** (each `xs: ref Vec<i64>, k: i64 -> Vec<i64>`). Rolling window of size k over a `Vec<i64>`; result is a fresh `Vec<i64>` of length `n - k + 1` (empty if `n < k` or `k <= 0`). C side uses O(n*k) per-window scan for max/min, plus an O(n) rolling accumulator for sum and an O(n*k) per-window product (multiply has no clean inverse on i64). LLVM side uses the simpler per-window form for all four (parity tests check output, not algorithm shape). Both backends byte-identical: with `xs=[1,5,3,2,4,6]` and k=3, max=[5,5,4,6], min=[1,2,2,2], sum=[9,10,9,12], product=[15,30,24,48]; oversized or zero/negative window → empty Vec. 1 new lib test. 1522 lib + 54 parity green.)
