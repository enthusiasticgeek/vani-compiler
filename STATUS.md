...
**Last updated:** 2026-06-01 (closures #588-#591 — RNG surface additions. **`rand_f64()`** uniform [0,1) via 53 high bits of `intent_rng_next()` divided by 2^53 (0x4340000000000000 in IEEE 754). **`rand_in_range_f64(lo, hi)`** = `lo + (hi-lo) * rand_f64()`. **`rand_bool()`** = low bit of `intent_rng_next()`. **`rand_choice(ref xs: Vec<i64>) -> i64`** uniform pick; empty Vec → -1. All four share the existing `intent_rng_next()` runtime (xoshiro stream). Both backends byte-identical given the same seed: with `seed_rng(42)`, the first `rand_choice([100,200,300])` picks 200 deterministically on both backends. 1 new lib test. 1538 lib + 54 parity green.)
