...
**Last updated:** 2026-06-01 (closures #528-#531 — **`vec_add_scalar`**, **`vec_sub_scalar`**, **`vec_mul_scalar`**, **`vec_div_scalar`** (each `xs: ref Vec<i64>, v: i64 -> Vec<i64>`). Scalar-broadcast element-wise arithmetic on `Vec<i64>`. `div_scalar` guards `v == 0` by returning an empty Vec instead of trapping on SIGFPE/UB. LLVM bodies emitted from one shared skeleton with `(op, prefix, ir_op, guards_zero)` quadruples; `add/sub/mul/sdiv` are the IR opcodes. Both backends byte-identical: with `xs=[10,20,30,40]`, `add(5)=[15,25,35,45]`, `sub(5)=[5,15,25,35]`, `mul(2)=[20,40,60,80]`, `div(10)=[1,2,3,4]`. Division by zero returns empty. 1 new lib test. 1524 lib + 54 parity green.)
