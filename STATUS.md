...
**Last updated:** 2026-06-01 (closure #592 — **`rand_normal() -> f64`** standard normal sample via Box-Muller transform. `u1 = rand_f64()` with re-sample loop guarding `u1 == 0` (avoids `log(0)`); `u2 = rand_f64()`; `z = sqrt(-2·log(u1)) · cos(2π·u2)`. The 2π constant uses IEEE 754 hex `0x401921FB54442D18`. Both backends byte-identical given the same seed: `seed_rng(7)` produces stream starting `[-6.07464, -0.737335, -0.295492, -0.990764, …]`. 1 new lib test. 1539 lib + 54 parity green.)
