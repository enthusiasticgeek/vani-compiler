...
**Last updated:** 2026-06-01 (closures #546-#549 — **`vec_mod_scalar`**, **`vec_pow_scalar`**, **`vec_shl_scalar`**, **`vec_shr_scalar`** (each `xs: ref Vec<i64>, v: i64 -> Vec<i64>`). Modular and bit-shift scalar broadcasts on `Vec<i64>`. Per-op guards: mod returns empty Vec if `v == 0`; pow returns empty if `k < 0` (repeated-multiply via inner loop); shl/shr return empty if `k < 0 || k > 63`. LLVM: mod/shl/shr share one shared scalar skeleton parameterized by `(op, prefix, ir_op, guard)` with a per-op guard fragment that may early-branch to the empty-return block; pow has its own outer/inner loop. Both backends byte-identical: `xs=[10,17,25,42]` mod 7 → `[3,3,4,0]`; squared → `[100,289,625,1764]`; `ys=[1,2,4,8]` shl 3 → `[8,16,32,64]`; shr 1 → `[0,1,2,4]`. Guarded inputs (mod 0, shl 100, pow -1) all return empty Vec safely. 1 new lib test. 1528 lib + 54 parity green.)
