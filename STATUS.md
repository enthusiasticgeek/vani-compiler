...
**Last updated:** 2026-06-01 (closures #566-#568 — **`vec_merge_sorted`** (`ref xs, ref ys -> Vec<i64>`, O(n+m) two-pointer merge of pre-sorted Vecs; if inputs aren't sorted, output won't be either — caller's responsibility), **`vec_insert_sorted`** (`ref xs, v -> Vec<i64>`, find first index where xs[i] >= v, splice v in; length n+1), **`vec_is_sorted_unique`** (`ref xs -> bool`, true iff strictly ascending — no equal-adjacent values). Both backends byte-identical: `xs=[1,3,5,7]` ⊎ `ys=[2,4,6,8]` → `[1,2,3,4,5,6,7,8]`; `insert_sorted(xs, 4) = [1,3,4,5,7]`, `insert_sorted(xs, 0) = [0,1,3,5,7]`, `insert_sorted(xs, 99) = [1,3,5,7,99]`; `is_sorted_unique([1,2,3,4])=true`, `[1,2,2,3]=false`, `[3,1,2]=false`. 1 new lib test. 1533 lib + 54 parity green.)
