...
**Last updated:** 2026-06-01 (closures #562-#565 — **`vec_count_distinct`** (`ref xs -> i64`, O(n²) unique-value count), **`vec_mean`** (`ref xs -> i64`, integer mean = sum/len, empty → 0), **`vec_indices_of_value`** (`ref xs, v -> Vec<i64>`, two-pass: count matches then fill), **`vec_dedup_consecutive`** (`ref xs -> Vec<i64>`, removes only adjacent duplicates — distinct from `vec_unique` which is general O(n²)). LLVM helpers all hand-rolled with proper allocas / select / icmp. Both backends byte-identical: `xs=[3,1,4,1,5,9,2,6,5]` → `count_distinct=7`, `mean=4` (sum 36 / len 9), `indices_of_value(5)=[4,8]`, miss → empty; `dups=[1,1,2,3,3,3,4,5,5]` → `dedup_consecutive=[1,2,3,4,5]`. Empty Vec → 0/empty cleanly. 1 new lib test. 1532 lib + 54 parity green.)
