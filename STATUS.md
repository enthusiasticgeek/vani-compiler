...
**Last updated:** 2026-06-01 (closures #594/#595 — **`vec_windows(xs: ref Vec<i64>, k: i64) -> Vec<Vec<i64>>`** overlapping length-k windows, `n-k+1` of them; empty if `n<k` or `k<=0`. **`vec_flatten(xss: ref Vec<Vec<i64>>) -> Vec<i64>`** two-pass concatenation: sum inner lengths, malloc total, copy. Both built on the Tier W nested-Vec construction pattern proven by #593. LLVM side fully inlines the IR — no external helper calls required. Cross-backend byte-identical: `xs=[1,2,3,4,5]` windows(3) → `[[1,2,3],[2,3,4],[3,4,5]]`; chunks(2) + flatten round-trips to `[1,2,3,4,5]`; oversize window → empty. `vec_group_by_value` deferred to a fresh session (its two-pass grouping is more involved than the others). 1 new lib test. 1541 lib + 54 parity green.)
