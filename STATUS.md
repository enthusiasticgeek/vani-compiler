...
**Last updated:** 2026-06-01 (closures #582-#587 — borrowed-Str classifier predicates returning bool. **`str_is_empty(s)`** = `s[0] == 0`. **`str_is_ascii(s)`** every byte < 128 (empty vacuously true). **`str_is_digit_only(s)`** / **`str_is_alpha_only(s)`** / **`str_is_alphanumeric_only(s)`** / **`str_is_whitespace_only(s)`** scan + bail; empty → false. C: inline statement-expressions with byte-walk loops. LLVM: hand-rolled scan-and-bail IR with per-call unique label tags (via `ctx.fresh_label`) to avoid SSA collisions on repeated calls. Per-byte predicate computed in i32 to allow common range-check patterns. Both backends byte-identical across positive/negative/empty cases for all six. 1 new lib test. 1537 lib + 54 parity green.)
