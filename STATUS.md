...
**Last updated:** 2026-06-01 (closures #541-#545 — **`vec_add_pairwise`**, **`vec_sub_pairwise`**, **`vec_mul_pairwise`**, **`vec_min_pairwise`**, **`vec_max_pairwise`** (each `xs, ys: ref Vec<i64> -> Vec<i64>`). Element-wise binary ops between two `Vec<i64>`s. Result length = `min(len(xs), len(ys))` — mismatched-length inputs truncate to the shorter side rather than padding (zip-style). New `check_vec_pairwise_builtin` checker (generalized from #324 vec_chain). LLVM bodies use one shared 2-input skeleton parameterized by `(op, prefix, body_fragment)` triples; min/max use `select` on `icmp`, the others use direct `add/sub/mul`. Both backends byte-identical: with `xs=[1,5,3,8]` and `ys=[2,4,7,1]`, `add=[3,9,10,9]`, `sub=[-1,1,-4,7]`, `mul=[2,20,21,8]`, `min=[1,4,3,1]`, `max=[2,5,7,8]`. Mismatched-length truncation verified. 1 new lib test. 1527 lib + 54 parity green.)
