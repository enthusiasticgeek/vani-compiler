...
**Last updated:** 2026-06-01 (closures #573-#576 — bit/byte packing. **`i64_byte_at(x, i)`** returns byte i of x (0..=7) as a 0-255 value; out-of-bounds i → 0 defensive. **`i64_set_byte(x, i, b)`** returns x with byte i set to (b & 0xFF); out-of-bounds i → x unchanged. **`i64_count_leading_ones(x)`** / **`i64_count_trailing_ones(x)`** count high/low contiguous 1-bits via `~x` then `clz`/`ctz`; guards `~x == 0` (all-ones input) → 64. C: inline statement-expressions using `__builtin_clzll`/`__builtin_ctzll`. LLVM: `@llvm.ctlz.i64`/`@llvm.cttz.i64` intrinsics already declared in preamble. Both backends byte-identical: `byte_at(0x1234567890ABCDEF, 0)=239 (0xEF)`, `byte_at(_, 7)=18 (0x12)`, out-of-bounds → 0; `clo(-1)=64`, `cto(-1)=64`, `cto(7)=3`. 1 new lib test. 1535 lib + 54 parity green.)
