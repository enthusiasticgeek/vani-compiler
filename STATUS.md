...
**Last updated:** 2026-06-01 (closures #538/#539/#540 — **`vec_min_with_scalar`**, **`vec_max_with_scalar`** (`xs: ref Vec<i64>, v: i64 -> Vec<i64>`) and **`vec_clamp_scalar`** (`xs: ref Vec<i64>, lo: i64, hi: i64 -> Vec<i64>`). Scalar floor / ceiling / clamp on `Vec<i64>`. `min_with_scalar`/`max_with_scalar` use the existing 2-arg `(ref Vec, i64)` family. `vec_clamp_scalar` is a new 3-arg checker (`check_vec_clamp_scalar_builtin`). LLVM bodies use branchless `select` over `icmp` (slt then sgt for clamp's two-stage min/max). Both backends byte-identical: with `xs=[1,5,10,3,8]` and v=5, `min_with_scalar=[1,5,5,3,5]`, `max_with_scalar=[5,5,10,5,8]`; with `xs=[-5,0,3,7,12,20]` clamped to `[0,10]` → `[0,0,3,7,10,10]`. 1 new lib test. 1526 lib + 54 parity green.)
