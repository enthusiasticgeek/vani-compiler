...
**Last updated:** 2026-06-01 (closures #524-#527 — **`vec_abs`**, **`vec_negate`**, **`vec_signum`**, **`vec_square`** (each `xs: ref Vec<i64> -> Vec<i64>`). Element-wise unary transforms; length preserved. C side uses a per-element loop with the obvious C operator. LLVM side uses one shared malloc-out-buf + per-elem-transform skeleton parameterized by a small IR fragment (per-op `transform` string substituted with the prefix). Both backends byte-identical: with `xs=[-3,0,5,-7]`, `abs=[3,0,5,7]`, `negate=[3,0,-5,7]`, `signum=[-1,0,1,-1]`, `square=[9,0,25,49]`. Empty Vec → empty result. 1 new lib test. 1523 lib + 54 parity green.)
