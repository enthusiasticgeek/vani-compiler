...
**Last updated:** 2026-06-01 (closure #596 — **`vec_group_by_value(xs: ref Vec<i64>) -> Vec<Vec<i64>>`**. Groups consecutive runs of equal values. Two-pass: count distinct runs (pass 1: walk pairs, increment ng on mismatch), allocate outer (ng * 24 bytes), then fill (pass 2: detect boundaries at `i==n` or `xs[i] != xs[i-1]`, malloc inner, copy run, write inner struct into outer.data[gi]). Both backends fully inline the IR. Both backends byte-identical: `xs=[1,1,2,3,3,3,5,1,1]` → 5 groups `[[1,1],[2],[3,3,3],[5],[1,1]]` (last `[1,1]` distinct from first); single-element → 1 group; empty → 0 groups. **Tier W complete (4/4 ops).** 1 new lib test. 1542 lib + 54 parity green.)
