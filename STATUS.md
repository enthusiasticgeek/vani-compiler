...
**Last updated:** 2026-06-01 (closures #550-#553 — **`vec_rotate_left`**, **`vec_rotate_right`**, **`vec_shift_left`**, **`vec_shift_right`** (each `xs: ref Vec<i64>, k: i64 -> Vec<i64>`). Positional (not bit-level) rotations/shifts. Rotations are cyclic: `xs=[1,2,3,4,5]` rl 2 → `[3,4,5,1,2]`, rr 2 → `[4,5,1,2,3]`. Shifts zero-fill the vacated end: sl 2 → `[3,4,5,0,0]`, sr 2 → `[0,0,1,2,3]`. `k >= n` for shifts → all zeros; `k mod n` for rotations. `k < 0` for any → empty Vec. LLVM: all 4 share one outer skeleton (k<0 guard, len==0 guard, malloc buf, outer index loop) with per-op body inlined (`urem` modulo for rotates, branchless `select` for shifts with safe src indexing to avoid OOB load). Both backends byte-identical including wrap (`k=7` on len-5 → `k mod n = 2`) and oversize shift (`k=100` → all zeros). 1 new lib test. 1529 lib + 54 parity green.)
