...
**Last updated:** 2026-06-01 (closures #569-#572 — sort-free analytics on `Vec<i64>`. **`vec_range_span`** (`ref xs -> i64`, max - min, single-pass; empty → 0). **`vec_mode`** (`ref xs -> i64`, O(n²) most-common value; ties → smallest; empty → 0). **`vec_kth_smallest`** (`ref xs, k -> i64`, 0-indexed; count-based O(n²) — for each candidate x in xs, fewer = #{xs[j] < x}, nm = #{xs[j] <= x}; the k-th smallest is the value where fewer <= k < nm; out-of-range k or empty → -1). **`vec_median`** (`ref xs -> i64`, lower median = `kth_smallest((n-1)/2)`; empty → 0). All four hand-rolled in LLVM IR (median composes via LLVM-level call to `@intent_vec_int64_t_kth_smallest`). Both backends byte-identical: `xs=[3,1,4,1,5,9,2,6,5]` (sorted = [1,1,2,3,4,5,5,6,9]) → `range_span=8`, `mode=1` (1 appears twice, ties with 5 but smaller), `median=4` (sorted[4]), `kth_4=4`, `kth_8=9`, `kth_oob=-1`. Even-length `[10,20,30,40]` → lower median = `sorted[1] = 20`. 1 new lib test. 1534 lib + 54 parity green.)
