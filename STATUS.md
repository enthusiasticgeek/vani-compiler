...
**Last updated:** 2026-06-01 (closures #532-#537 — **`vec_eq_mask`**, **`vec_ne_mask`**, **`vec_lt_mask`**, **`vec_le_mask`**, **`vec_gt_mask`**, **`vec_ge_mask`** (each `xs: ref Vec<i64>, v: i64 -> Vec<i64>`). Scalar comparison masks: each returns a fresh `Vec<i64>` of 0/1 elements where 1 indicates the comparison holds at that index. Useful for branchless / SIMD-style programming and as building blocks for `vec_filter`-style flows that the closure surface doesn't yet support. C side uses ternary `?:`. LLVM emits one shared skeleton parameterized by `(op, prefix, cmp_pred)` triples — `icmp <pred>` produces an `i1`, `zext i1 -> i64` materializes the 0/1 element. Both backends byte-identical: with `xs=[1,5,5,7,3]` and v=5, eq=[0,1,1,0,0], ne complement, lt/le/gt/ge all correct including ties. Empty Vec → empty result. 1 new lib test. 1525 lib + 54 parity green.)
