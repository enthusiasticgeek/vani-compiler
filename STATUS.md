...
**Last updated:** 2026-06-01 (closures #554-#557 — **`vec_subset_of`**, **`vec_disjoint`**, **`vec_equal_set`**, **`vec_equal_seq`** (each `xs, ys: ref Vec<i64> -> bool`). Dual-Vec bool predicates: `subset_of` checks every elt of xs appears in ys (set membership); `disjoint` checks no elt appears in both; `equal_set` composes `subset_of(xs,ys) && subset_of(ys,xs)` (mult-insensitive); `equal_seq` checks same length AND `xs[i] == ys[i]` for all i (mult-sensitive). New `check_vec_dual_predicate_builtin` checker (parallel to pairwise but Bool-returning). LLVM: equal_seq is a length-equal + iter loop; subset_of and disjoint are O(n*m) nested-loop scans with early-exit; equal_set composes via two `@intent_vec_int64_t_subset_of` calls. Both backends byte-identical: `xs=[1,2,3] ⊆ ys=[1,2,3,4,5]` ✓; `xs ⫩ zs=[4,5,6]` ✓; `equal_set(xs, reverse(xs))=1` (same set) but `equal_seq=0` (order differs). 1 new lib test. 1530 lib + 54 parity green.)
