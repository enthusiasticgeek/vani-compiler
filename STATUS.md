...
**Last updated:** 2026-06-01 (closure #593 — **`vec_chunks(xs: ref Vec<i64>, k: i64) -> Vec<Vec<i64>>`** Tier W. Splits xs into consecutive length-k chunks (last may be shorter). The first builtin to *construct* a `Vec<Vec<i64>>` programmatically. C side: dedicated `intent_vec_vec_int64_t_chunks` helper, gated by `program_uses_vec_chunks` (so the helper only emits when Vec<Vec<i64>> is actually declared). LLVM side: inline IR expansion at the call site — outer malloc + per-chunk inner malloc + write 3-field inner struct via getelementptr/store into outer's data slot. Affine drop machinery for the nested Vec already exists (each inner Vec's data buffer is freed when the outer drops). Both backends byte-identical: `xs=[1..=7]` chunked by 3 → `[[1,2,3],[4,5,6],[7]]`; empty input → empty outer. 1 new lib test. 1540 lib + 54 parity green.)
