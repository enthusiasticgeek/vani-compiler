...
**Last updated:** 2026-06-01 (closures #558-#561 — **`vec_diff`** (`ref xs -> Vec<i64>`, length n-1, consecutive differences), **`vec_pad_left`** / **`vec_pad_right`** (`ref xs, target_len, fill -> Vec<i64>`, pad to max(target_len, n) with fill at the requested end — no truncation), **`vec_replace_value`** (`ref xs, old, new -> Vec<i64>`, fresh Vec with every occurrence of `old` replaced by `new`, length unchanged). `vec_diff` goes in the existing 1-arg unary family; the 3-arg ones piggyback on the generalized `check_vec_clamp_scalar_builtin` checker. Both backends byte-identical: `xs=[1,3,7,4,10]` → `diff=[2,4,-3,6]`; `ys=[1,2,3]` → `pad_left(5,9)=[9,9,1,2,3]`, `pad_right(5,9)=[1,2,3,9,9]`, `pad_left(2,0)=[1,2,3]` (no truncation); `xs` with replace 7→99 → `[1,3,99,4,10]`. 1 new lib test. 1531 lib + 54 parity green.)
