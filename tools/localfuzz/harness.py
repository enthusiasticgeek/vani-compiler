...
Runs inside the vani-fuzz container (see docker-compose.yml). Each cycle it
either mutates a corpus .vani file or (every HARNESS_GENERATE_EVERY cycles)
asks the local Ollama model to write a fresh one, then runs the candidate
through `vanic check` and both backends of `vanic run`. Anything that
crashes, hangs, or diverges between backends gets saved under
tools/localfuzz/findings/ and drafted into docs/TODO_LOCAL_STAGING.md.
