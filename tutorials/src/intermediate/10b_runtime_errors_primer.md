...
1. **LLVM backend (`vanic run`'s default)**: `assert` and
   ...
   trap.
2. **C backend (`--backend=c`)**: `assert`, `requires`, bounds
   check, and overflow / divide-by-zero / shift ALL exit cleanly
   with code `3` now, matching the LLVM backend exactly, on every
   trap category. (The `ref Vec<T>` `requires` case used to hit a
   different, older code path and raise a real `SIGABRT` via a raw
   glibc `assert()` macro instead; fixed 2026-08-07 -- see Row 5
   below. Overflow / divide-by-zero / shift used to call a raw
   libc `abort()` instead of `exit(3)` -- deliberately left that
   way when the BUG-106-class fixes converted `assert`/`requires`,
   since those fixes targeted a misleading-crash-banner problem
   specific to `lli`-JIT on the LLVM side that the C backend's
   plain `SIGABRT` never had. It turned out to be a real
   backend-divergence bug anyway -- the two backends disagreeing
   on exit code for the identical trap -- caught automatically by
   `tools/backend_crosscheck.py`'s corpus-wide sweep on its very
   first run and fixed the same day (#191, 2026-08-14). Bounds
   check was the last holdout: that sweep mistakenly assumed
   bounds-check was consistently `abort()`-based on every backend
   and left it alone, but both LLVM backends' own bounds-check
   helper had used `exit(3)` since BUG-108/162 -- only the C
   backends never got the memo. A localfuzz finding caught the gap
   directly (BUG-194, 2026-08-15); bounds check now calls `exit(3)`
   too, on both C backends.)
   All four trap categories now agree on exit code `3` across both
   backends. Check the STDERR MESSAGE TEXT (present on BOTH
   backends as of BUG-162 -- see the wording caveat above for why
   the exact text can still differ) if you need to detect which
   check actually fired -- the exit code alone no longer
   distinguishes bounds from the other three, or C from LLVM.
   ...
   these 4 traps, so buffered `print` output survives the crash on
   both backends either way.
...
- **Logic-class crashes** are **compile-time when SMT can
  ...
  on the C backend, `assert` / `requires` / bounds / overflow /
  div-by-zero / shift ALL exit(3) with a message too now (the
  overflow/div-by-zero/shift trio joined as of #191, 2026-08-14;
  bounds joined last, as of BUG-194, 2026-08-15) -- see "Row 2"
  above for the verified details and why the message WORDING can
  still differ between backends even though both now print
  something. `ensures` AND `invariant` now both fall back to a
