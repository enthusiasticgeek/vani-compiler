# Intermediate 5 â€” Dynamic dispatch: `dyn Iface` + `Vec<dyn Iface>`
...
> **New to this?** Read [Intermediate 4a â€” What's a `dyn Iface`? primer](04a_dyn_iface_primer.md)
...
one â€” the right formula is picked at runtime by following the
directory. This is *dynamic dispatch*.

## The program

```vani
intent "Intermediate 5 worked example â€” dyn Iface + heterogeneous Vec.";
...
fn main() -> i64 {
  ...
  // Implicit T â†’ dyn Iface coercion at the call site.
  print "circle =", area_of(c);
  print "square =", area_of(s);

  // Heterogeneous Vec â€” the canonical dyn dispatch use case.
...
- **Implicit `T â†’ dyn Iface` coercion** kicks in at let-bindings,
...
If none of those apply, the static-dispatch generic in [Â§4](04_generics_iface.md)
is faster and simpler â€” no vtable indirection.
...
- **`Vec<dyn Iface>` as a struct field across multiple Iface
  ...
  that shipped in Phase 1.2 â€” make sure your compiler is up to
  date if you hit confusing C-codegen errors when storing two
  different `Vec<dyn â€¦>` fields.
- **Methods in the interface declare a concrete receiver
  type** (`self: Circle`) â€” that's the v1 static-dispatch
...
**Next**: [Â§6 â€” Closures and iterator combinators â†’](06_closures.md)
