# Advanced 10 â€” Compiler internals tour

> **Learning goal**: orient yourself in the vÄá¹‡à¥€ compiler
...
1. **Lexer** â€” reads characters, groups them into tokens
   (`let`, `42`, `+`). Like a copy editor who circles every
   word on the page.
2. **Parser** â€” reads tokens, builds a tree of the program's
   structure (the AST). Like the editor grouping circled words
   into sentences and paragraphs.
3. **Checker** â€” verifies types, ownership, contract
   obligations. Like a fact-checker who verifies every claim
   before publication.
4. **Codegen** â€” translates the checked tree into LLVM IR or C.
   Like a translator who renders the document in another
   language.
5. **Backend** â€” LLVM compiles IR to machine code; `cc`
   compiles C.
...
.vani source
    â”‚
    â”‚  src/lexer.rs        â€” tokens + script-purity gate
    â–¼
  tokens
    â”‚
    â”‚  src/parser.rs       â€” recursive-descent â†’ ast::Program
    â–¼
   AST  (src/ast.rs)
    â”‚
    â”‚  src/checker.rs      â€” type-check + affine ownership +
    â”‚                        SMT discharge (via src/smt.rs)
    â–¼
  TypedProgram  (src/ir.rs)
    â”‚
    â”‚  src/lower.rs        â€” TypedProgram â†’ SSA Module (for
    â”‚                        the SSA backends; tree-* skip this)
    â–¼
   IR Module  (src/ir.rs)
    â”‚
    â”œâ”€â–º src/backend_c.rs       â€” tree-C emit  (canonical C)
    â”œâ”€â–º src/ssa_backend_c.rs   â€” SSA-C emit (preferred when supported)
    â”œâ”€â–º src/backend_llvm.rs    â€” tree-LLVM emit (canonical LLVM IR)
    â””â”€â–º src/ssa_backend_llvm.rs â€” SSA-LLVM emit
```
...
| `src/lower.rs` | TypedProgram â†’ SSA Module | `lower_program`, `lower_expr` |
...
4. **PRELUDE injection** (`src/lib.rs::inject_prelude`). Lexes
   ...
   *Note: this used to clobber the PrintLangMode set in step
   3 â€” Phase 1.1 fixed it by saving/restoring around the
   inject call.*
...
|---|---|---|
| Lib tests (~1900) | `src/lib.rs` | Per-pass unit tests â€” checker, lower, codegen shape pins, regression for each phase |
...
- **Lexer**: `src/lexer.rs` â€” `DialectLang`, `Script`,
  `PrintLangMode`, `enforce_language_purity`, per-script
  `*_keyword` functions.
- **Diagnostic**: `src/diagnostic.rs` â€” `DiagLang`,
...
**Congratulations â€” you've completed the Advanced track!**

That's the whole tutorial set (Beginner + Intermediate +
Advanced â€” 34 lessons). The next-best thing is to:
