...
| MATH-3 | `vanic run`'s JIT reports a **failed `assert`** as a native stack overflow (Windows `STATUS_STACK_BUFFER_OVERRUN`, exit 9) instead of a clean non-zero exit. Found while debugging vani-optimize's Hessian test: an `assert close(...)` was genuinely failing (finite-difference error ~2.04e-6 against a 1.0e-6 tolerance -- a test bug, since fixed), but under `vanic run`/`vanic test` it manifested as an apparent crash rather than an assertion failure, which sent the investigation down the wrong path for a while. `vanic build`'s AOT binary reports the exact same failing assert correctly as exit code 3. Likely the JIT's assert-failure trap unwinds through a path that corrupts/overruns the JIT's execution stack rather than calling the same clean-exit routine the AOT backend uses. Not blocking (every package can avoid it by validating logic via `vanic build` first when a `vanic run`/`test` result looks like a crash), but worth fixing since a stack-overflow-looking exit code is actively misleading. | ~2-4 h | nothing |
