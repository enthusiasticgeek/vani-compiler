...
> Session-by-session history extracted from STATUS.md on 2026-06-19.
> Pre-Arc-8 session logs, design notes, and historical work queues.
> Current state lives in [STATUS.md](STATUS.md).
...
Gurmukhi all land on top of the per-script abstraction set up
in Phase 5b. Each gets its own Unicode block check, ~30-keyword
starter alias table, native-script error labels, and a
numeral-print helper emitting digits at the script's own
Unicode codepoints (Tamil ௦..௯, Telugu ౦..౯, Gujarati ૦..૯,
Gurmukhi ੦..੯).

Refactor highlight: the per-script numeral-print helpers (C +
LLVM, tree + SSA) are now generated by parameterized
emitters keyed on (suffix, lead_byte). Adding the remaining
Phase 6 scripts (Kannada, Malayalam, Odia, Assamese, Sinhala)
is now ~10 lines of mechanical change per language. The
script-purity gate also collapsed from N²-style per-pair
checks to one pass tracking "first observed script."

Lib ledger: 1909 (was 1904) = 5 new dialect tests including a
cross-script (Tamil-in-Telugu) rejection regression. Parity
sweep covers 62 examples internally (was 58).
...
Closes [docs/v1_limitations.md L11](docs/v1_limitations.md).

**Phase 1.2 — C-backend `Vec<dyn Iface>` struct-field fix**:
A struct with two distinct `Vec<dyn …>` fields used to collapse
both bundles to the same C typedef (`intent_vec_intent_dyn`) and
also emit the bundle BEFORE the dyn fat-pointer typedef. Now:
...
**Last updated:** 2026-06-06 evening. Tip-of-tree summary:
  - **Tests**: 1897 lib + 54 parity green on Linux.
