...
> Full historical work queue extracted from TODO.md on 2026-06-19.
> Pre-v0.1.0 work items, closed closures, and planning notes.
> The current actionable work queue is in [docs/TODO_CURRENT.md](docs/TODO_CURRENT.md).
...
| ~~**Kosh: private-registry TLS `cafile` support**~~ | ✅ DONE commit `cc9293d` | `VANI_HTTP_CAINFO` env var + `[registry] cafile` in `vani.toml` → `--cacert` in every curl call. Thread-local carries cafile through call tree. |
...
| ~~**Kosh: read `dl` URL from `config.json` at runtime**~~ | ✅ DONE commit `cc9293d` | `fetch_registry_dl_template` fetches config.json at `vanic add`/`vanic update` time; falls back to hardcoded constant. CDN migration now needs only a config.json edit, not a compiler release. |
