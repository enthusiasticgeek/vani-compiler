...
| ~~**Phase 4 — Vec<ref T>**~~ | ✅ SHIPPED 2026-06-09 | — | Per-shape Vec typedef + helpers on both backends (`intent_vec_ref_<inner_tag>` / `intent_vec_refmut_<inner_tag>`); `element_tag` / `vec_struct_tag` route `Type::Ref(_)` / `Type::RefMut(_)`; `c_element_storage` returns `T*`; LLVM `vec_struct_tag` mirrors with `ref_<tag>` / `refmut_<tag>`. Scope-escape analyzer extended at the `push(mut ref vec, ref X)` call site (rejects when X is at deeper scope than the Vec). Lifts the type-validator + vec-literal rejections at three sites. 5 new lib tests pin accept (same-scope literal + push, mut-ref variant, Vec<ref P> struct field) + reject (deeper-scope push) shapes. `examples/vec_of_ref.vani` ASan-clean cross-backend stdout parity. |
...
| Task | Effort | Scope |
|---|---|---|
| ~~**L4 (B) Phase 4 — `Vec<ref T>`**~~ | ✅ SHIPPED 2026-06-09 | Per-shape Vec typedef + helpers on both backends; element_tag / vec_struct_tag handle Ref/RefMut; scope-escape analyzer at push call sites. 5 lib tests + acceptance example (ASan-clean). The actual scope was smaller than the 6-10h investigation: ~3h focused work since the existing Copy-element bundle path covered refs once `element_tag` and `c_element_storage` returned valid identifiers/types. |
| ~~**L4 (C) — Returning refs directly (`fn foo() -> ref T`)**~~ | ✅ SHIPPED 2026-06-09 | Single-ref-param elision rule (Swift/OCaml-style): returns a ref whose lifetime is inferred from the single ref parameter. Zero/multi-ref-param returns reject with clear diagnostics. New `Signature.elided_ref_source` field tracks the source param index; new `VarInfo.ref_aliases` tracks per-binding lifetime source chains; new `collect_var_ref_aliases` analyzer chases Var ref-types through their alias chains at every escape site (Return, FieldAssign, Vec-push). Both backends already lowered ref params as `T*` so return-position codegen was a no-op. Acceptance: `examples/path_c_ref_returns.vani` cross-backend parity-green. 5 new lib tests pin accept (single ref param, same-scope call) + reject (zero ref params, multi-ref params, deeper-scope chained call) shapes. The actual scope was 2-3h focused work since path-B's escape analyzer machinery extended naturally. **L4 (B+C) is now fully closed.** Path-D territory (multi-input distinct lifetimes, lifetime-parameterized structs, closures capturing refs that outlive) remains deferred indefinitely. |

### 📋 User-direction items (added 2026-06-08, end-of-session)

Three feature requests from the user. Each is a substantive
session-scale arc; effort estimates are honest first-pass.

| Task | Effort | Scope |
|---|---|---|
| **Error-message elaboration — break diagnostics into step-by-step explanation** | 8-15h, multi-session | Today's diagnostics are usually one-line factual: "let initializer must be assignable to OwnedStr, got Str". Goal: when an error is non-obvious, the message walks the user through the diagnosis — WHAT went wrong, WHY the compiler thinks so, HOW to fix it. Pattern: attach "elaboration" lines that, when an explainer is registered for a diagnostic family, render as a numbered breakdown after the standard message. The vast majority of diagnostic sites would NOT need elaboration; pick the 20-30 most-common ones to seed. Touches `src/diagnostic.rs` (carry elaboration vec) + every error-emit site that we improve. |
| **Big-O annotation in compilation output (flag-gated, default on)** | 12-20h, dedicated arc | When the compiler emits artifacts (with `--big-o` flag, on by default), each user-declared fn gets a Big-O complexity annotation derived statically from the body: loop nesting depth + recursive-call depth + container-op asymptotics (push = O(1) amortized, sort = O(n log n), etc.). Annotated in `vanic check` output AND in a per-fn comment block above each fn's emit. Flag: `--big-o=auto` (default-on for fns with loops/recursion; off for trivial), `--big-o=force` (annotate everything), `--big-o=off`. Failure mode: unable-to-prove-bound — annotate as `O(?)` with the unprovable construct flagged. SMT-discharged: the existing Z3 integration already handles `requires`/`ensures`; bound-derivation extends that. Real research work; pin a v1 scope (loop-nesting-only, builtin asymptotics) before committing to the full lift. |
| **Tutorials — explain CS concepts for non-CS-bg readers** | 20-40h, multi-session arc | The mdBook tutorials assume some CS familiarity. Goal: rewrite Beginner + Intermediate tracks to be welcoming to readers with NO CS background. Each new CS concept introduced via a relatable analogy + worked example before the formal definition. Topics flagged: pointers / references / mutable references / heap vs stack / `interface` / `dyn Iface` / `Box` / RAII / modules / namespaces / lifetimes / affine ownership / SMT / parallelism. Each should be a self-contained short chapter. Pattern: open with "imagine you're …", give a 1-paragraph analogy, then walk through a tiny vāṇी example, then explain what just happened in CS terms. Hand-curate for engaging prose; don't auto-generate. |
| **`volatile_read` / `volatile_write` built-ins (embedded MMIO)** | 4-6h, single session | **Why not C-style `volatile T`:** type-qualifier `volatile` infects the type system (volatile pointers, pointers-to-volatile, volatile struct fields — a known C footgun), encourages the `volatile int flag` threading anti-pattern (wrong — use `Atomic<T>`), and doesn't compose with affine ownership. **Design (Rust-inspired):** two `unsafe` built-in *operations*, not a storage qualifier. `volatile_read(ref mmio_reg): T` emits `load volatile` (LLVM) / `*(volatile T*)ptr` (C backend). `volatile_write(mut ref mmio_reg, val: T)` emits `store volatile` / `*(volatile T*)ptr = val`. The volatile property belongs to the *access*, not the *variable*. **Scope:** (1) parse `volatile_read` / `volatile_write` as built-in call exprs (no new keyword); (2) checker: require `unsafe { }` block, embedded build triple only — reject on hosted Linux/macOS/Windows with clear diagnostic "volatile_read requires an embedded build triple; use Atomic<T> for threading"; (3) LLVM backend: emit `load volatile` / `store volatile` IR; (4) C backend: emit `*(volatile T*)` cast-read/write; (5) add `examples/embedded/mmio_blink.vani` smoke example + 3 lib tests (accept inside unsafe+embedded, reject on hosted, reject outside unsafe). **MMIO wrapper pattern** (recommend in docs): `struct MmioReg<T> { addr: i64 }` with `fn read(ref self) -> T { unsafe { volatile_read(...) } }` / `fn write(mut ref self, v: T) { unsafe { volatile_write(..., v) } }` — so callers never spell `volatile_read` directly. **Deferred:** `volatile` fence / memory-barrier ordering (needs a separate `memory_fence(SeqCst)` built-in arc). |
...
| Task | Effort | Notes |
|---|---|---|
| ~~LSP — Mandarin keyword autocomplete + `?` hover docs~~ | ✅ SHIPPED 2026-06-08 | — | Pragma-aware completion: `// vani-lang: mandarin` surfaces ~55 Mandarin keywords; `// vani-lang: sanskrit/hindi/marathi/nepali/maithili/konkani` surfaces a Devanagari Indo-Aryan keyword union. Default (no pragma) stays English-only. English `KEYWORDS` list also expanded from ~25 to ~50 entries. Hover docs: cursor-on-`?` returns the operator's doc block plus the type. Works on broken documents (lex-only path) and when the parse-time desugar moved the typed expr's span. |
| ~~LSP — VS Code grammar (TextMate / tree-sitter) sync~~ | ✅ N/A | The repo doesn't ship a separate `vani.tmLanguage.json` or tree-sitter grammar — the LSP semantic-tokens provider in `src/lsp.rs` is the canonical source. Mandarin keywords already auto-highlight (they route through existing TokenKinds). Postfix `?` was added to the operator-skip list 2026-06-08. |
| ~~v3.1 caveat #2 — liveness initial-impl polish~~ | ✅ SHIPPED 2026-06-08 | — | Probed affine-across-suspend shapes; found a real bug: `v31_default_init_expr` returned `ExprKind::Str("")` for OwnedStr fields, which types as Str → coerce fail at struct-lit init. Fix: synthesize `"" + ""` (Str+Str concat returns OwnedStr). Three regression tests pin OwnedStr / Vec / multi-suspend-with-affine-between shapes. The deeper optimization ("only locals KEPT ACROSS an await land in the state struct") remains future work; conservative-correct behavior (every local in state struct) is verified safe. |
| ~~v3.1 caveat #15 — test-surface polish~~ | ✅ SHIPPED 2026-06-08 | — | Added three structural snapshot tests (`v31_snapshot_linear_i64_async_fn_synthesizes_canonical_task_shape`, `v31_snapshot_non_i64_return_uses_result_field_aba`, `v31_snapshot_a44_canceltoken_ref_param_field_is_refmut_struct`) that walk `checked.ir.structs` / `.functions` and assert on the synthesized Task struct fields + poll fn signature directly. Catches ABI shifts that substring-on-emitted-C tests would miss. |
| ~~Devanagari counterparts (non-async)~~ | ✅ SHIPPED 2026-06-08 | — | `examples/language/sanskrit/try_question_op.vani` + `box_recursive_drop.vani` shipped + e2e parity green. Async counterparts (echo_pool, async_cancel_auto) deferred to the "async/Task__ dialect lift" item below. |
| ~~Async / Task__ dialect lift~~ | ✅ SHIPPED 2026-06-08 | — | Path (a) shipped: `is_async_ident` + `is_await_ident` helpers in `src/parser.rs` recognize per-dialect spellings (Sanskrit / Hindi / Marathi: अतुल्यकालिक + प्रतीक्षा; Mandarin: 异步 + 等候; Japanese: 非同期 + 待機). Ships `examples/language/sanskrit/async_cancel_auto.vani` (smoke) + `examples/language/mandarin/async_cancel_auto.vani` (full A4.4 surface). |
| ~~`Task__` / `__poll_` mangled-name prefix in dialect~~ | ✅ WON'T FIX (v1) | — | The synthesized name prefixes stay English by design. The synthesized struct + poll fn are INTERNAL ABI emitted by the v3.1 transform; they're not user-authored identifiers. The function-name SUFFIX preserves the user's chosen spelling (e.g. `Task__आनयक` if the user named their async fn `आनयक`), so the dialect's chosen identifier name is honored. A v2 lift could add a `// vani-mangle-prefix: कार्य_` pragma for users who want a fully-Devanagari surface, but it's not blocking any user-facing capability today. |
| **Grammar-consultant pass: review dialect spellings** | external/ongoing | Native-speaker linguist review of: (1) the per-dialect async/await spellings shipped 2026-06-08 (अतुल्यकालिक / प्रतीक्षा / 异步 / 等候 / 非同期 / 待機) — chosen for face-validity as tatsama coinages or widely-attested CS spellings, NOT validated for native-speaker active-use idiom; (2) the ~55 Mandarin keywords in `mandarin_keyword` (`src/lexer.rs`); (3) every other non-English keyword table. The linguist's revisions can be applied as a single sweep when contact is established. |
...
**Shipped — 61 dialects across 26 scripts. The Phase 13 long-
tail is closed; Mandarin remains as the only queued language
(in the dedicated-session lifts table above due to the CJK
tokenizer requirement).**

#### Indic + Perso-Arabic (21 dialects)
Sanskrit, Hindi, Marathi, Nepali, Maithili, Konkani-Devanagari,
Bengali, Assamese, Tamil, Telugu, Gujarati, Punjabi (Gurmukhi +
Shahmukhi), Kannada, Malayalam, Odia, Sinhala, Urdu, Sindhi,
Persian, Pashto.

#### Latin Tier II + Tier III (22 dialects)
English, Spanish, French, German, Italian, Portuguese, Polish,
Turkish, Vietnamese, Romanian, Dutch, Hungarian, Czech, Slovak,
Swedish, Norwegian, Danish, Finnish, Catalan, Yoruba, Hausa,
Indonesian / Malay / Filipino / Swahili (basic-Latin pragma-
threaded).

#### Non-Latin global (18 dialects across 12 new scripts)
Russian (Cyrillic), Japanese (3-block CJK collapsed), Korean
(Hangul), Modern Standard Arabic (existing Arabic script),
Greek, Hebrew, Thai, Khmer, Burmese, Amharic (Ethiopic),
Tibetan, Cherokee, Lao, Mongolian (traditional), Armenian,
Georgian.

#### Architectural milestones that made the rollout possible
- **Phase 5b (2026-06-07)**: per-script abstraction
  (`Script::X` + `DialectLang::X` + `DiagLang::X` + classify
  range + pragma + keyword table). The 6-touchpoint mechanical
  pattern repeated 60+ times.
- **Pragma threading (commit `c3a2bb6`, 2026-06-08)**: `Lexer.pragma`
  cached at construction → per-dialect ASCII keyword tables
  gated on pragma. Unblocked basic-Latin dialects.
- **Unified `lex_ident` (Phase 8b.1)**: consumes non-ASCII
  continuation bytes so Latin-with-accents (Spanish, French,
  German, Portuguese, Italian, Polish, Czech, Hungarian,
  Romanian, Slovak, Swedish, Norwegian, Danish, Turkish,
  Vietnamese, Catalan, Yoruba) all ride one entry point.

#### Optionally queued — not in the current dedicated-session table
- **Yiddish** (Hebrew script, shares table with Hebrew, ~1-2h
  to add a `DialectLang::Yiddish` alias if requested).
- **Cyrillic Mongolian** (~2-3h; rides Cyrillic script slot
  alongside Russian; would need pragma-gated keyword table
  since both share Script::Cyrillic).
- **Persian-script Mongolian / Manchu / Uyghur Arabic** —
  Perso-Arabic script with non-Indic languages; small effort
  on existing infra.
...
examples; **1928 lib + 54 parity green as of 2026-06-06 evening**;
...
platforms), re-run the full Linux suite to confirm:
  - No regressions in the existing 1928 lib tests.
...
- [ARC8_V3_PLAN.md](ARC8_V3_PLAN.md) — phased execution plan
  ...
  3-returns + 3-params ✅ COMPLETE 2026-06-04** — compiler-driven
  `async fn → Task` transform handles linear bodies +
  non-suspending control flow + suspend-in-branch state-
  splitting + fall-through merge state + nested ifs in
  suspending branches + ANF lifting + match arms WITH
  suspends (Int + Bool + Str + Float + Variant +
  VariantWithBinding + Wildcard) + loops with suspend in
  body + break/continue inside loops + non-i64 locals
  (bool / f64 / Str / OwnedStr / Enum / Vec / Struct /
  Array) + **non-i64 return types** (bool / Str / Enum /
  Vec / Struct / Array via `__result: T` field). Shipped
  ...
  `3bbbe83`, `e6f747f`, `72fd8e7`, plus pending 3-params.
  **v3.1 feature-complete: control-flow + non-i64 locals +
  non-i64 returns + non-i64 PARAMS + `try` keyword for
  Result propagation.** **Next session recommended: Phase 5
...
| **Arc 8 v1.6** | Full TCP networking builtin family (`tcp_listen`/`accept`/`connect_local`/`send_str`/`recv`/`send_buf`/`close`/`socket_port`) + `examples/tcp_echo.vani` + `examples/tcp_multi_echo.vani` (3 concurrent clients) + str-literal-in-task-body walker fix | ✅ COMPLETE — commits `9aaec41`, `83010ab` shipped 2026-06-04 |
| **Arc 8 v2 runtime** | epoll family (`epoll_new`/`add_read`/`wait_one`/`close`) + non-blocking I/O (`tcp_set_nonblocking`/`tcp_accept_nb`/`tcp_recv_nb`) + `examples/tcp_echo_epoll.vani` single-threaded cooperative 3-concurrent-client echo on ONE OS thread | ✅ COMPLETE — commit `92864de` shipped 2026-06-04 |
| **Arc 8 v3 pattern** | Async-flavored aliases (`io_recv_async`/`io_send_async`/`io_accept_async`) + `examples/tcp_echo_state_machine.vani` hand-rolled state-machine pattern (struct + poll fn + driver loop) | ✅ COMPLETE — commit `f7743a1` shipped 2026-06-04 |
| **Arc 8 v3.1 sugar** | Compiler-driven `async fn` → `Task<T>` transform: parser-level scan of async-fn bodies, auto-synthesis of struct/poll-fn/constructor triples, thread-local registry flush into program decls. | **OPTIONAL — sugar transform that automates v3 pattern.** Multi-day compiler work (~500 lines parser + tests + IR); doesn't fit a single session. Underlying capability + pattern already ship. **15 design caveats** captured in STATUS.md's "v3.1 design caveats" table — body shape (linear only initially), local liveness, affine types across await, ANF lifting for multiple awaits, ref param handling, CancelToken plumbing, error propagation, nested async-fn calls, generics, etc. |
| **Arc 8 platform port** | Lift the Linux-only restriction on Arc 8 v1.5/v1.6/v2/v3 runtime helpers. macOS needs kqueue shim + `__error()` errno; Windows needs full IOCP port + winsock + `_errno()`. | **OPEN — gated on cross-platform CI availability** (same gate as Arc 7 Win64/AArch64). macOS ~8-12h focused; Windows ~25-35h. Until then, a compile-time gate should fail loud on non-Linux targets (~1h follow-up). See STATUS.md's "🪟 Platform support" block. |
...
23. **Graph (general)** — ✅ AFFINE.
    ...
    *Reverse-CSR + Prim promotion shipped 2026-05-29 (closure
    #338).* Three new struct fields (rev_adj_start /
    rev_adj_csr_src / rev_adj_csr_weight) appended at indices
    9/10/11; byte_size 72 → 96. A new build_rev_csr_if_needed
    helper builds the reverse adjacency cache (keyed on
    destination) via the same two-pass count-prefix-scatter
    pattern. Prim now builds both CSRs and iterates u's
    incident edges as the union of forward (u→v) and reverse
    (v→u) CSR walks — O(degree) per pop. Every graph
    algorithm except Kruskal now does O(V+E) adjacency work.
    Kruskal stays per-edge by design (sorts the edge list).
    Pending: A* path reconstruction (currently returns just
    the total cost, not the path); undirected variant of the
    type itself; edges visiting via closures; generic weight
    types.
...
mangling: public items mangle to `<mod>__<name>`
(matches parser output for `mod::name`); private
items mangle to `<mod>__priv__<name>` (form the
parser CAN'T produce). Outside references hit a
lookup miss. New `PRIVATE_MODULE_ITEMS` thread-local
in ast.rs maps mangled-private names to source-form
paths; the unknown-function diagnostic consults it
to surface "function 'mod::name' is private to its
module — mark it 'pub' to allow access from outside"
instead of the cryptic mangled form. v1 enforces fn
visibility; struct / enum / const / type-alias
visibility (same mechanism, parallel wiring) is a
follow-up Phase 2.1. Two new lib tests cover
inside-allowed and outside-blocked cases. Test
totals: 929 lib + 47 e2e + 11 vtables-phase3 + 2
user-drop-by-ref + 1 ssa-examples.
...
declarators can't appear prefix-only. Fix: emit
`void*` for the return slot when it's itself a
FnPtr (same indistinguishability convention as
#214/#215). Test totals: 908 lib + 47 e2e passing.
#215 tree-LLVM Vec<FnPtr> tag fix: parallel to #214 on
the LLVM side. `vec_struct_tag` fell through to
`llvm_type(FnPtr)` which is `unreachable!` → panic
during emit. Added `Type::FnPtr` arm returning
`"fnptr"`. Test totals: 907 lib + 47 e2e passing.
...
`inject_branch_drops`. Non-Copy arg like OwnedStr →
callee consumed the heap AND caller's scope-exit Drop
fired on the same binding → ASan double-free. Regular
`check_call` had the consume + inject pair; #213
mirrors it. Test totals: 905 lib + 47 e2e passing.
#212 SSA-LLVM gates out Vec<Atomic/Channel>: SSA-LLVM
represents Atomic<T> as the alloca *pointer*; storing
a pointer-shaped SSA value into an `i32` Vec slot
emitted `store i32 %ptr, …` and failed the LLVM IR
verifier. Tree-LLVM doesn't have this issue. Added
`stmt_uses_vec_of_atomic_or_channel` to
`ssa_llvm_extra_reject` so programs touching
`Vec<Atomic<T>>` / `Vec<Channel<T,N>>` route through
tree-LLVM. SSA-C unaffected. Test totals: 904 lib + 47
e2e passing.
...
the parent's const qualifier → `const Mutex*` mismatch
vs the helper's non-const param → -Wdiscarded-qualifiers.
Closure #176 covered direct ref params; #210 covers
field-borrow through `ref Struct`. Emits
`(intent_mutex_i64*)&v_c->lock` const-strip cast. Test
totals: 903 lib + 47 e2e passing.
...
a Copy-but-user-Drop struct (e.g. `Resource { id: i64 }`
plus `implement Drop`), the per-field walk emitted
nothing and the user's drop was silently skipped at
Block-expr scope exit. Regular stmt-level Drop already
had the user-Drop check; the inline Block arm needed
the same. Test totals: 900 lib + 47 e2e passing.
...
warned `-Wunused-label`. Fix: pre-scan terminators (Jump,
Branch) plus special-region targets (parallel-for exit,
multi-block task end) to build a `referenced_blocks` set;
emit labels only for those. All 58 examples now clean
under -Wunused-label. Test totals: 897 lib + 47 e2e
passing.
...
- **Namespaces / modules.** *Phase 1 done 2026-05-26
  (closure #242)*: parser + checker flatten module blocks.
  *Phase 2 (visibility enforcement) done 2026-05-26
  (closure #243)*: non-`pub` items mangle to
