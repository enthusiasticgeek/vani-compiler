...
| **Phase 4 — Vec<ref T>** | 4-6h | Deferred from this session because Vec<ref T> safety needs additional analysis at FN-CALL sites that mutate Vecs (push / insert / set) — tracking whether the Vec outlives the ref being stored. The simple FieldAssign check doesn't catch `push(mut ref outer_vec, ref inner_local)`. Not blocking; the existing Vec-element-cannot-be-ref rejection stays for now. |
