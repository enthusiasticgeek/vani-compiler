...
| **Arc 8 v3 sugar** | State-machine codegen — `async fn` bodies auto-rewrite to poll-functions over the epoll reactor + `intent_event_loop_run<T>(task) -> T` driver + `examples/tcp_echo_async.vani` | **OPTIONAL — sugar layer, not a missing capability.** Underlying single-thread multiplexing already ships via v2 runtime; v3 is the compiler-driven ergonomics polish. See STATUS.md's "📋 Arc 8 v3" block. |
