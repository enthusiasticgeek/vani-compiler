...
| **Async / Task__ dialect lift** | 4-6h | The `async` keyword is contextual-English-only today (checked via the literal string `"async"` as an Ident token in `check_async_prefix`). The v3.1 transform also synthesizes English-looking names like `Task__<fn>` and `__poll_<fn>` that appear in user code. To ship clean Devanagari/Mandarin counterparts of `echo_pool.vani` + `async_cancel_auto.vani`, either: (a) accept per-dialect spellings of `async` as contextual keywords, or (b) document the mixed-content convention. Optional polish; doesn't affect the async surface itself. |
