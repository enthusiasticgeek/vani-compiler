...
| **Arc 8 v2 runtime** | State-machine codegen (8c) + event-loop runtime (8d) + non-blocking I/O variants of sleep_ms/tcp_* (8e proper) + cooperative-fan-out example (8h proper) | **OPEN — performance/ergonomics arc, not a missing user feature.** Arc 8 is capability-complete today: async surface + sleep_ms + full TCP + task+join concurrency. v2 upgrades thread-per-task + blocking I/O to single-thread cooperative scheduling + non-blocking I/O. See STATUS.md's "📋 NEXT SESSION" block. |
