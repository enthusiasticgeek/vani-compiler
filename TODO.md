...
| **Kosh: private-registry TLS `cafile` support** | ~2h | `VANI_HTTP_CAINFO` env var + `[registry] cafile` field in `vani.toml`, forwarded as `--cacert <path>` to every `curl` call in `src/manifest.rs`. Needed for users whose internal registry uses a self-signed cert or an org CA not in the OS cert store. Design fully spec'd in `SECURITY.md`. Mirrors cargo's `[http] cainfo`. |
| **Kosh: Sigstore / cosign package signing** | multi-day | Per-package signing at `vanic publish` using short-lived OIDC certs; signature bundle recorded in Rekor transparency log; `vanic add` verifies signature at download time. Defer until there is a meaningful publisher community. Full design in `SECURITY.md`. Mirrors Crates.io RFC #3721 and npm provenance attestations. |
| **Kosh: read `dl` URL from `config.json` at runtime** | ~2h | Fetch `config.json`'s `dl` field at `vanic add` time (cache in `vani.lock`) instead of the hardcoded `DEFAULT_DL_TEMPLATE` constant. Lets `config.json` updates propagate to all clients without a compiler release — needed before Phase 2 CDN migration so the switch is one `config.json` edit, not a coordinated compiler release. See `docs/registry_migration.md`. |
| **Kosh: Phase 1 registry migration (org transfer)** | ~1h | Transfer `kosh-index` to a `vani-lang` GitHub org; update 3 constants in `manifest.rs`. Trigger: first external contributors or shared maintainership needed. No size benefit, full governance benefit. Full playbook in `docs/registry_migration.md`. |
