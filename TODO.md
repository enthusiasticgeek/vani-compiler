...
| **Arc 8 v1.6** | Full TCP networking builtin family (`tcp_listen`/`accept`/`connect_local`/`send_str`/`recv`/`send_buf`/`close`/`socket_port`) + `examples/tcp_echo.vani` echo server + str-literal-in-task-body walker fix | ✅ COMPLETE — commit `9aaec41` shipped 2026-06-04 |
| **Arc 8 runtime** | State-machine codegen (8c) + event-loop runtime (8d) + non-blocking I/O variants of sleep_ms/tcp_* (8e proper) + cooperative-fan-out example (8h proper) | **OPEN — focused multi-day arc, picked up in next session.** See STATUS.md's "📋 NEXT SESSION" block. v1.5/v1.6 above give users blocking timer-driven async + real TCP networking + task-based concurrency TODAY; runtime arc upgrades to single-threaded cooperative scheduling on one thread. |
