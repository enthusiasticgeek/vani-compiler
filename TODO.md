...
| **3** | **L4 (B) Phase 3+4 — ref struct fields + Vec elements** | 5-10h **+ Phase 2 analyzer**, one or two sessions | Investigation 2026-06-08: Phase 1 shipped because every escape vector for a let-bound ref still had a type-level rejection (fn return / struct field / Vec element). Phases 3+4 LIFT two of those rejections, which opens new escape vectors — a fn can return `MyStruct { item: ref local_foo }`, or a Vec<ref Foo> can outlive its source. So Phase 3+4 needs the scope-escape analyzer the design doc anticipated. Conservative v1 rule: a binding that holds a ref to `foo` must be in `foo`'s declaring scope or an inner scope; reject cross-scope storage. Mostly checker work + clear diagnostics; both backends already handle ref struct fields (via the v3.1 Task__ partial lift). |
