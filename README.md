...
- **Arc 8 FULLY COMPLETE — async + networking + concurrency + state machines end-to-end**:
  ...
  - **Arc 8 v3.1 sugar OPTIONAL**: parser-level compiler transform that auto-rewrites `async fn` bodies with `io_*_async` calls into the state-machine struct/poll/constructor triples users today write by hand. Multi-day compiler work; underlying pattern + capability already ship. **15 design caveats documented** — see [STATUS.md](STATUS.md) *v3.1 design caveats* (linear-body-only initial scope, local liveness, affine types across await, ANF lifting for multi-await, ref param handling, CancelToken plumbing, error propagation, nested async calls, generics, multi-task scheduling, timerfd-based `sleep_ms_async`, diagnostic quality, test surface).
...
**Test ledger at 2026-06-04: 1817 lib + 54 parity green** (post Arc 8 v3 — Arc 8 FULLY COMPLETE).
