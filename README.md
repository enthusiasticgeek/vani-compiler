...
- **Arc 8 FULLY COMPLETE — async + networking + concurrency + state machines end-to-end**:
  ...
  - **Arc 8 v3.1 sugar — SUBSTANTIALLY SHIPPED 2026-06-04**: parser-level compiler transform that auto-rewrites `async fn` bodies with `io_*_async` calls into the state-machine struct/poll/constructor triples users previously wrote by hand. Phase 0 + 1 + 2 narrow + 2.1a-c + 2.2 + 2.3-narrow + 2.3a/b/c/d + 2.5 + 2.5b shipped — covers linear bodies, non-suspending control flow, suspend-in-branch state-splitting, fall-through merge state, nested ifs, ANF lifting, loops with suspend, break/continue, AND **match-with-suspends FEATURE-COMPLETE** for every literal + enum pattern shape (Int / Bool / Str / Float / Variant / VariantWithBinding / Wildcard) via desugaring + tag-extraction sub-matches + inline substitution. 17 acceptance examples cross-backend parity-green. See [ARC8_V3_PLAN.md](ARC8_V3_PLAN.md) for the phased plan-of-record. Phase 2.4 (`try` keyword) + Phase 3 (affine types across await — non-i64 locals + OwnedStr/Vec) + Phase 4 (generics + nested async + multi-task) queued.
...
**Test ledger at 2026-06-04: 1855 lib + 57 parity green** (post Arc 8 v3.1 Phase 2.3d — match-with-suspends feature-complete).
