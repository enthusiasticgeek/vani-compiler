...
- **External plan-of-record docs at `~/vani/`** (refreshed 2026-06-04):
  - `~/vani/ARC8_V3_PLAN.md` — **phased execution plan** for Arc 8 v3.1 compiler-driven sugar (5 phases, ~78-98h) + Arc 8 platform port (2 phases, ~35-50h). **Phase 0 + Phase 1 + Phase 2 narrow ✅ COMPLETE 2026-06-04** — compiler-driven `async fn → Task` transform handles linear bodies AND if/while/Assign/Print + mid-body return (when branches don't suspend). examples/tcp_echo_async + echo_with_timeout parity-green. Next session: Phase 5 (macOS port ~10-15h) or Phase 2.1 (suspend-in-branch state-splitting ~10-12h).
