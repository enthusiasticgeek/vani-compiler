...
- [vāṇī async / asyncio design](project_vani_async_design.md) — compiler-lowered state machines on arena; explicitly NOT Pin / self-references. **Arc 8 FULLY COMPLETE 2026-06-04** — v1 source surface + v1.5 timers + v1.6 blocking TCP + v2 epoll + non-blocking I/O runtime. Four parity-green examples ship: `async_io` (timer), `tcp_echo` (1 client), `tcp_multi_echo` (3 task clients), `tcp_echo_epoll` (3 clients on ONE thread via the epoll reactor). State-machine sugar (compiler auto-rewriting `async fn` bodies into poll-functions over the epoll reactor) is an OPTIONAL v3 polish — the underlying capability already ships
