...
- [vāṇī async / asyncio design](project_vani_async_design.md) — compiler-lowered state machines on arena; explicitly NOT Pin / self-references. **Arc 8 v1 + v1.5 + v1.6 ✅ shipped** — v1 source surface (`async fn`, `await`, `Future<T>`, `Poll<T>`, `CancelToken`) + v1.5 `sleep_ms` + `examples/async_io.vani` + v1.6 full TCP networking family (`tcp_listen`/`accept`/`connect_local`/`send_str`/`recv`/`send_buf`/`close`/`socket_port`) + `examples/tcp_echo.vani`. Arc 8 runtime (state-machine codegen + epoll event loop + non-blocking I/O variants) queued for next session
