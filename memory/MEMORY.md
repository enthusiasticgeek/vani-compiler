...
- **External plan-of-record docs at `~/vani/`** (refreshed 2026-06-04):
  - `~/vani/ARC8_V3_PLAN.md` — **phased execution plan** for Arc 8 v3.1 compiler-driven sugar (5 phases, ~78-98h) + Arc 8 platform port (2 phases, ~35-50h). **Phase 0 + 1 + 2 narrow + 2.1a + 2.1b + 2.1c + 2.2 + 2.3-narrow + 2.3a + 2.3b + 2.3c + 2.3d + 2.5 + 2.5b + 3a + 3b + 3c + 3d + 3e + 3f + 3-returns ✅ COMPLETE 2026-06-04** — compiler-driven `async fn → Task` transform handles linear bodies + non-suspending control flow + suspend-in-branch state-splitting + fall-through merge state + nested ifs + ANF lifting + match arms WITH suspends (Int + Bool + Str + Float + Variant + VariantWithBinding + Wildcard) + loops with suspend in body + break/continue inside loops + non-i64 locals (bool / f64 / Str / OwnedStr / Enum / Vec<T> / Struct / Array) + **non-i64 return types** (same composite set via `__result: T` field + status-only poll ABI). **v3.1 affine-types across the entire async-fn boundary: locals + returns; params still i64-only.** 24 v3.1 acceptance examples parity-green. Phase 3e shipped C-backend payloaded-enum-typedef ordering fix; Phase 3f shipped C-backend memcpy escape hatch for array FieldAssign. **Phase 2.4 (try keyword) NOW UNBLOCKED.** Next session: Phase 2.4 try keyword (~6-8h) or Phase 5 macOS port (~10-15h).
