...
- **External plan-of-record docs at `~/vani/`** (refreshed 2026-06-04):
  - `~/vani/ARC8_V3_PLAN.md` — **phased execution plan** for Arc 8 v3.1 compiler-driven sugar (5 phases, ~78-98h) + Arc 8 platform port (2 phases, ~35-50h). **Phase 0 + 1 + 2 narrow + 2.1a + 2.1b ✅ COMPLETE 2026-06-04** — compiler-driven `async fn → Task` transform handles linear bodies + non-suspending control flow + suspend-in-branch state-splitting + fall-through merge state. 6 v3.1 acceptance examples parity-green. Next session: Phase 5 (macOS port ~10-15h) or Phase 2.2 (ANF lifting ~5-6h) or Phase 2.1c (nested ifs ~5-8h).
