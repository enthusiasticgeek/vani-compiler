...
- **External plan-of-record docs at `~/vani/`** (refreshed 2026-06-04):
  - `~/vani/ARC8_V3_PLAN.md` — **phased execution plan** for Arc 8 v3.1 compiler-driven sugar (5 phases, ~78-98h) + Arc 8 platform port (2 phases, ~35-50h). **Phase 0 + Phase 1 ✅ COMPLETE 2026-06-04** — compiler-driven `async fn → Task` transform now generates state-machine struct + poll fn + constructor for linear async-fn bodies. examples/tcp_echo_async.vani parity-green. Next session: Phase 5 (macOS port ~10-15h, parallel-safe) or Phase 2 (control flow inside async bodies ~15-20h).
