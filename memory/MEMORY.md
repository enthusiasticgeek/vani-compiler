...
- [vāṇī async / asyncio design](project_vani_async_design.md) — compiler-lowered state machines on arena; explicitly NOT Pin / self-references. **Arc 8 CAPABILITY-COMPLETE** — v1 source surface (`async fn`, `await`, `Future<T>`, `Poll<T>`, `CancelToken`) + v1.5 `sleep_ms` + `examples/async_io.vani` + v1.6 full TCP networking family + `examples/tcp_echo.vani` + multi-client `examples/tcp_multi_echo.vani`. Arc 8 v2 runtime (state-machine codegen + epoll event loop + non-blocking I/O variants → single-thread cooperative scheduling) is a performance optimization, not a missing user feature; queued for next session
