...
- **External plan-of-record docs at `~/vani/`** (refreshed 2026-06-04):
  - `~/vani/ARC8_V3_PLAN.md` — **phased execution plan** for Arc 8 v3.1 compiler-driven sugar (5 phases, ~78-98h) + Arc 8 platform port (2 phases, ~35-50h). **Phase 0 + 1 + 2 narrow + 2.1a + 2.1b + 2.1c + 2.2 + 2.3-narrow + 2.3a + 2.3b + 2.3c + 2.3d + 2.4 + 2.5 + 2.5b + 3a + 3b + 3c + 3d + 3e + 3f + 3-returns + 3-params + 4a-narrow + 4a-broad + 4b + 4c-narrow ✅ COMPLETE 2026-06-04 — Phase 4 single+multi-task arc COMPLETE**. 28 v3.1 acceptance examples parity-green; 1883 lib + 72 parity total; clean warning-free build. Phase 4c-broad scaffolded but reverted (commit `bd67dce`) — needs vāṇī mono-internals work.
  - **📋 NEXT SESSION HANDOFF**: `~/vani/TODO.md` § *NEXT SESSION HANDOFF* has the 18-item dependency-ordered remaining work split into three tiers: **Tier 1 = Phase 4c-broad (testable on Linux)**, Tier 2 = Phase 5 macOS port (code on Linux; parity verification needs macOS), Tier 3 = Phase 6 Windows IOCP (heaviest; needs Windows). Pick Tier 1 first — only tier where verification doesn't need platform access.
