...
- [vāṇī async / asyncio design](project_vani_async_design.md) — compiler-lowered state machines on arena; explicitly NOT Pin / self-references. **Arc 8 v1 ✅ shipped (source surface)** — `async fn`, `await(expr)`, `Future<T>`, `Poll<T>`, `CancelToken` at parser+prelude level with synchronous semantics. Arc 8 runtime (state-machine codegen + event loop + non-blocking I/O) queued for next session
