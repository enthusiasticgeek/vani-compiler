...
- **External plan-of-record docs at `~/vani/`** (refreshed 2026-06-04):
  - `~/vani/ARC8_V3_PLAN.md` — **phased execution plan** for Arc 8 v3.1 compiler-driven sugar (5 phases, ~78-98h) + Arc 8 platform port (2 phases, ~35-50h). **Phase 0 + 1 + 2 narrow + 2.1a + 2.1b + 2.1c + 2.2 + 2.3-narrow + 2.3a + 2.3b + 2.5 + 2.5b ✅ COMPLETE 2026-06-04** — compiler-driven `async fn → Task` transform handles linear bodies + non-suspending control flow + suspend-in-branch state-splitting + fall-through merge state + nested ifs + ANF lifting + match arms WITH suspends (Int + Bool + Str + Float + Wildcard) + loops with suspend in body + break/continue inside loops. 15 v3.1 acceptance examples parity-green. Next session: Phase 5 (macOS port ~10-15h) or Phase 2.4 (try keyword ~6-8h) or Phase 2.3c (Variant patterns ~6-10h).
