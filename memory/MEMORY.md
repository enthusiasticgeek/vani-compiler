...
- [vāṇī async / asyncio design](project_vani_async_design.md) — compiler-lowered state machines on arena; explicitly NOT Pin / self-references. **Arc 8 FULLY COMPLETE 2026-06-04** — v1 source surface + v1.5 timers + v1.6 blocking TCP + v2 epoll + non-blocking I/O runtime + v3 async-flavored aliases + hand-rolled state-machine pattern. Five parity-green examples ship: `async_io`, `tcp_echo`, `tcp_multi_echo`, `tcp_echo_epoll`, `tcp_echo_state_machine`. v3.1 compiler-driven sugar (auto-rewriting `async fn` bodies into struct/poll/constructor triples) is OPTIONAL multi-day work — pattern + capability already ship
