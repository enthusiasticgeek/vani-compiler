...
- [vāṇī async / asyncio design](project_vani_async_design.md) — compiler-lowered state machines on arena; explicitly NOT Pin / self-references. **Arc 8 v1 + v1.5 ✅ shipped** — v1 source surface (`async fn`, `await`, `Future<T>`, `Poll<T>`, `CancelToken`) + v1.5 `sleep_ms` builtin + `examples/async_io.vani` with timer-driven async + task-based concurrent fan-out (real OS threads). Arc 8 runtime (state-machine codegen + epoll event loop + non-blocking I/O futures) queued for next session
