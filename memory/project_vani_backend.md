...
- **Arc 8 v1.6 — full TCP networking builtin family** (commits
  ...
  IR builds `sockaddr_in` by hand. Walker fix in
  `collect_print_strings`: recurses into `TaskSpawn` +
  `UnsafeBlock` bodies so string literals inside task bodies
  intern at module scope. Examples: `tcp_echo.vani` (single
  client), `tcp_multi_echo.vani` (3 concurrent task clients).
- **Arc 8 v2 — epoll + non-blocking I/O runtime** (commit
  `92864de`). Seven new builtins: `epoll_new` /
  `epoll_add_read` / `epoll_wait_one` / `epoll_close` /
  `tcp_set_nonblocking` / `tcp_accept_nb` / `tcp_recv_nb`.
  All wrap libc epoll_create1 / epoll_ctl / epoll_wait /
